...
public class FlightArrivalAdapter extends BaseAdapter {
    ...
    public FlightArrivalAdapter(Context context, List<DataFlightSchedule> results)
    {
...
//			holder.imgBatt = (ImageView)convertView.findViewById(R.id.img_batt);
        ...
        holder.txtname.setText(searchArrayList.get(position).getAirlineName()+" "+searchArrayList.get(position).getFlightIataNumber()+" "+searchArrayList.get(position).getIataCode());
        holder.txtvalue.setText(searchArrayList.get(position).getScheduledTime());
